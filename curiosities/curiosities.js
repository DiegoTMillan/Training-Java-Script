...
let curiosities = [
  ...
  "Una de las teorías más aceptadas es que todo el tiempo pasado, presente y futuro, está contenido en el universo. Es por ello que en lo que a viajes en el tiempo se refiere, si viajaras atrás a matar a tu padre. Todo encajaría de algún modo para que el presente no cambiara en absoluto.",
  ...
  "El 99,9% de los átomos es espacio vacío. Si puediera suprimirse, toda la humanidad cabría en un terrón de azúcar ¿de verdad estamos tan vacíos?",
  ...
  "debemos nuestra energía a las madres. Todas nuestras mitocodondrias, proceden exclusivamente, del óvulo. El espermatozoide los pierde en el momento de la fecundación. De hecho tus mitocondrias poseen un ADN propio que, más allá de posibles mutaciones, es el mismo que el que poseen las mitocondrias de tu madre.",
...
// let miliseconds = parseInt("");
p.innerText = curiosities[randomNumber(0, curiosities.length - 1)];
// console.log(p.innerText.length)

// if (p.innerText.length > 420) {
//   miliseconds = 20000;
// } else if (p.innerText.length < 420 && p.innerText.length > 315) {
//   miliseconds = 3000;
// } else if (p.innerText.length < 315 && p.innerText.length > 130) {
//   miliseconds = 10000;
// } else {
//   miliseconds = 5000;
// }
...
let timerDiv = setInterval(
  () =>
    document.body.firstElementChild.firstElementChild.classList.toggle(
      "alternative"
    ),
  15000
);
